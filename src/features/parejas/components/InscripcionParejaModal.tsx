...
export const InscripcionParejaModal: React.FC<InscripcionParejaModalProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-700/80 rounded-xl p-5 text-white">
      <div className="mb-4 border-b border-slate-800 pb-3">
        <h3 className="text-xl font-bold text-blue-400">
          {competencia.categoria || competencia.nombre}
        </h3>
        <p className="text-xs text-slate-400 mt-1">
          Valor de Inscripción: <strong className="text-green-400">${competencia.precio || 0} por pareja</strong>
        </p>
      </div>
      ...
      {!busquedaRealizada ? (
        <div className="space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">DNI Jugador 1 (Solo números)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">DNI Jugador 2 (Solo números)</label>
            <input
              type="text"
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {(!j1Existe || !j2Existe) && (
            <div className="bg-blue-500/10 border border-blue-500/30 text-blue-300 text-xs p-3 rounded-xl">
              ℹ️ Uno o ambos DNI no están en el padrón. Registro rápido activo: completa los datos mínimos para competir.
            </div>
          )}
          ...
          <div className="bg-slate-800/60 p-4 rounded-xl border border-slate-700/60">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-bold text-blue-400">JUGADOR 1 (DNI: {dni1})</span>
              <span className={`text-xs px-2 py-0.5 rounded ${j1Existe ? "bg-green-500/10 text-green-400" : "bg-yellow-500/10 text-yellow-400"}`}>
          ...
          <div className="bg-slate-800/60 p-4 rounded-xl border border-slate-700/60">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-bold text-blue-400">JUGADOR 2 (DNI: {dni2})</span>
              <span className={`text-xs px-2 py-0.5 rounded ${j2Existe ? "bg-green-500/10 text-green-400" : "bg-yellow-500/10 text-yellow-400"}`}>
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
            ...
            <span className="text-[11px] text-slate-500 block mt-1">
              Si realizaste la transferencia al Alias del organizador, adjunta la referencia para acelerar la confirmación.
            </span>
          </div>
