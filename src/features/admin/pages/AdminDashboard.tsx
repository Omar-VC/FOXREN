...
import { GestionCircuitos } from "./GestionCircuitos"; // 👈 1. IMPORTAR
...
export const AdminDashboard: React.FC = () => {
  ...
  const handleAprobar = async (solicitud: any) => {
    ...
    try {
      ...
      alert(`Jugador ${solicitud.nombre} ${solicitud.apellido} aprobado con éxito en categoría ${catOficial}.`);
    } catch (error) {
      console.error(error);
  ...
  if (loadingPendientes || loadingActivos) return <p className="text-white p-6">Cargando dashboard...</p>;
    ...
    <div className="flex h-screen bg-gray-900 text-white">
        ...
        <nav className="flex flex-col space-y-2">
          <button
            ...
            className={`text-left px-3 py-2 rounded flex justify-between items-center ${
              seccionActiva === "solicitudes" ? "bg-gray-700 font-semibold" : "hover:bg-gray-700"
            }`}
          >
          ...
          <button
            ...
            className={`text-left px-3 py-2 rounded ${
              seccionActiva === "jugadores" ? "bg-gray-700 font-semibold" : "hover:bg-gray-700"
            }`}
          >
            Lista oficial
          </button>

          {/* 👈 3. NUEVO BOTÓN EN LA BARRA LATERAL */}
          <button
            onClick={() => setSeccionActiva("circuitos")}
            className={`text-left px-3 py-2 rounded ${
              seccionActiva === "circuitos" ? "bg-gray-700 font-semibold" : "hover:bg-gray-700"
            }`}
          >
            Circuitos y Llaves
          </button>
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        ...
        {seccionActiva === "solicitudes" && (
          <>
            <h1 className="text-2xl font-bold mb-4">Solicitudes de Registro Pendientes</h1>
            {pendientes.length === 0 ? (
              <p className="text-gray-400">No hay solicitudes pendientes de aprobación.</p>
            ) : (
              <ul className="space-y-3">
                {pendientes.map((sol) => (
                  <li key={sol.id} className="bg-gray-800 p-4 rounded-lg flex justify-between items-center border border-gray-700">
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm text-yellow-400">
                        Nivel / Cat. declarada: {sol.nivelInicial || sol.categoriaDeclarada || "No especificado"}
                      </span>
                    </div>

                    <div className="flex items-center space-x-3">
                      <div className="flex flex-col">
                        <label className="text-xs text-gray-400 mb-1">Cat. Oficial FOXREN</label>
                        <select
          ...
          <>
            ...
            <ul className="space-y-2">
              {jugadoresFiltrados.map((j) => (
                <li key={j.id} className="bg-gray-800 p-3 rounded flex justify-between items-center border border-gray-700">
                  <div className="flex flex-col">
                    <span className="font-semibold">
                      {j.nombre} {j.apellido} - DNI {j.dni}
                    </span>
                    <span className="text-sm text-gray-400">
                      Categoría oficial: <strong className="text-blue-400">{j.categoriaId || "Sin categoría"}</strong> | Estado: {j.estado}
                    </span>
                  </div>
              ...
              {jugadoresFiltrados.length === 0 && (
                <p className="text-gray-400">No se encontraron jugadores que coincidan con la búsqueda.</p>
              )}
            </ul>
          </>
        )}

        {/* 👈 4. MOSTRAR LA PANTALLA DE CIRCUITOS */}
        ...
        {jugadorSeleccionado && (
          <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
            ...
            <div className="flex space-x-2 mt-6">
              <button onClick={guardarCambios} className="bg-green-600 px-4 py-2 rounded hover:opacity-80 font-medium">
                Guardar cambios
              </button>
              <button onClick={() => setJugadorSeleccionado(null)} className="bg-gray-600 px-4 py-2 rounded hover:opacity-80 font-medium">
                Cancelar
              </button>
            </div>
          </div>
